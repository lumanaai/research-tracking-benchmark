...
class STrack(BaseTrack):
    ...
    def predict(self):
        ...
        if self.state != TrackState.Tracked:
            mean_state[7] = 0
        # apply class-aware dt
        self.kalman_filter.set_dt(self.dt)
    ...
    def activate(self, kalman_filter, frame_id):
        ...
        self.dt = DT_CLASS.get(self.class_id, 1.0)
...
class Fasttracker(object):
    ...
    def update(self, output_results, img_info, img_size):
        ...
        class_ids = output_results[:, -1].astype(int)
        # class_ids = np.random.randint(0, 9, size=output_results.shape[0])
        ...
        if len(dets) > 0:
            ...
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, cls_id) 
              for (tlbr, s, cls_id) in zip(dets, scores_keep, class_ids[remain_inds])]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            '''Detections'''
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, cls_id)
                    for (tlbr, s, cls_id) in zip(dets_second, scores_second, class_ids[inds_second])]
        else:
            detections_second = []
