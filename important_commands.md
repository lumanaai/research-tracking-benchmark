...
Trackers: `fasttracker`, `ocsort`, `hybridsort` (motion-only). `traffictrack` is still a stub (`TODOs.md`).
...
| Full track+metrics | `-m mot_pipeline.run all --benchmark … --tracker {fasttracker,ocsort,hybridsort} --detector …` |
