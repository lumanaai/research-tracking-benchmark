...
def scale_bboxes(img1_shape, bboxes, img0_shape, ratio_pad=None):
    ...
    bboxes[::2] -= pad[0]  # x padding
    bboxes[1::2] -= pad[1]  # y padding
    bboxes /= gain
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=300, mapping=None):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        c = c * (0 if agnostic else max_wh)  # classes
        ...
        if (time.time() - t) > time_limit:
            LOGGER.debug(f'WARNING: NMS time limit {time_limit:.3f}s exceeded')
