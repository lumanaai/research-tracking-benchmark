...
def load_text_nn(model, weights, half, device):
    ckpt = torch.load(weights, map_location=lambda storage, loc: storage)
    if "state_dict" in ckpt.keys():
        ckpt = remove_prefix(ckpt["state_dict"], "module.")
    else:
        ckpt = remove_prefix(ckpt, "module.")
        ...
        for i in range(mat.shape[0]):
            ...
            # with separators
                ...
                for word in words:
                    ...
                    t = ctcBeamSearch(
                        matrix, self.character, self.ignore_idx, None, beamWidth=beamWidth, dict_list=dict_list
                    )
                    string += t
            texts.append(string)
        return texts
...
def group_text_box(
    polys, slope_ths=0.1, ycenter_ths=0.5, height_ths=0.5, width_ths=1.0, add_margin=0.05, sort_output=True
):
