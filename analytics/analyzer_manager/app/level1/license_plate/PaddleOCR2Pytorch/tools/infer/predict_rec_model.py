    ...
    def __call__(self, img_list):
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            if self.rec_algorithm == "SRN":
                ...
                if self.use_gpu:
                    inp = [e_i.to(f'cuda:{self.device}') for e_i in inp]
                with torch.no_grad():
...
def main(args):
    ...
    last_part_time = time.time() - last_part_time
    total_time += last_part_time
    print("Predict time of {} images time: {:.3f}, avg time: {} ms".format(
        len(img_list), total_time, 1000 * (total_time) / len(img_list)))
