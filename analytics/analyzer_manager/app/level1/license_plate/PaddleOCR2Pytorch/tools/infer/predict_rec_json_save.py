...
def clean_license_plate(ocr_result):
    cleaned = ''.join(c for c in ocr_result if c.isalnum())
    return cleaned
...
def main(args):
    ...
    end_time = time.time()
    print("Predict time of {} images time: {:.3f}, avg time: {} ms".format(
        len(img_list), end_time - st, 1000 * (end_time - st) / len(img_list)))

    # Get output_json_path from args, default to './output/results.json'
    ...
    print("Results saved to {}".format(output_json_path))
...
if __name__ == '__main__':
    ...
    # # Add default output_json_path if not present
    # if not hasattr(args, 'output_json_path'):
    #     args.output_json_path = './output/results.json'
...
# python ./tools/infer/predict_rec_json_save.py \
...
#     --rec_model_path /home/noamr/analytics_research/our_ocr/PaddleOCR2Pytorch/en_ptocr_v4_rec_infer.pth \
#     --rec_yaml_path /home/noamr/analytics_research/our_ocr/PaddleOCR/configs/rec/PP-OCRv4/en_PP-OCRv4_rec_LPR.yml \
#     --rec_image_shape='3,48,320' \
#     --rec_char_dict_path  /home/noamr/analytics_research/our_ocr/PaddleOCR/ppocr/utils/en_dict.txt
