...
class BaseRecLabelDecode(object):
    ...
    def decode(self, text_index, text_prob=None, is_remove_duplicate=False):
        ...
        for batch_idx in range(batch_size):
            ...
            text = "".join(char_list)
            if text_prob is not None:
                valid_idxs = np.logical_or(text_prob[batch_idx] < 0.9, text_index[batch_idx] > 0)
                conf = np.mean(text_prob[batch_idx][valid_idxs])
            else:
    ...
    def get_beg_end_flag_idx(self, beg_or_end, char_or_elem):
        if char_or_elem == "char":
            if beg_or_end == "beg":
                ...
                assert False, "Unsupport type %s in get_beg_end_flag_idx of char" % beg_or_end
            ...
            if beg_or_end == "beg":
                ...
                assert False, "Unsupport type %s in get_beg_end_flag_idx of elem" % beg_or_end
        else:
