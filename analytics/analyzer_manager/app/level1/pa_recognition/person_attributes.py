...
class PaConfig(ClassifierAConfig):
    weights: str = "pa_efficientnet_b4_calib_2_1.pt"  # path for builtin weight file
    attr_desc_file: str = "pa_net_2.1.csv"  # path for network description file
    ...
    name: InferenceType = InferenceType.PERSON_ATTR


class AttributesModel(BaseAttributesModel):
    _config_type = PaConfig
    args: PaConfig
...
# class PaConfig(BaseInferenceConfig):
#     weights: str = "pa_efficientnet_b4_calib_2_1.pt"  # path for builtin weight file
#     attr_desc_file: str = "pa_net_2.1.csv"  # path for network description file
#     im_size: Tuple[int, int] = (224, 224)
#     no_sqr_pad: bool = False  # Anti aliasing when down sampling
...
#     fields: List[PersonAttribute]
#     binary_fields: List[PersonAttribute]

#     def __init__(self, label, fields: List[PersonAttribute], global_fields_map: Dict[int, PersonAttribute]):
#         self.label = label
...
#     @staticmethod
#     def gen_binary_attr(field: PersonAttribute, score: float, priority) -> AttrProperty:
...
#             self._global_field: PersonAttribute = global_category[0].binary_fields[0]
#         else:
...
#         mapping: Dict[int, PersonAttribute] = {}
#         categories: Dict[str, List[PersonAttribute]] = {}

#         with open(file_name, "rt") as f:
#             table = csv.DictReader(f)
#             for row in table:
#                 attr = PersonAttribute(**row)
...
#     def _build_categories(self, categories: Dict[str, List[PersonAttribute]]):
...
# class AttributesModel(InferenceWrapper):
#     _config_type = PaConfig
#     args: PaConfig
#     interpreter_class: type = AttributeInterpreter
#     _interpreter: AttributeInterpreter
